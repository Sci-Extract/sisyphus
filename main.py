...
system_message = \
"""You are a expert text classifier. The task given to you is a binary problem.
"""
prompt_cls = \
"""Classify below text quoted by triple backticks based on whether the text is talking about nonlinear optical crystals properties, which include at least one of these properties: \
second harmonic generation, birefringence, cut-off edge (absorption edge), band gaps (Eg), laser damage threshold.
Here are some examples
Text: In this work, we developed a new DUV birefringent crystal LiBO2 based on [BO2]∞ infinite chains in the Li-B-O system, which simultaneously achieves the shortest UV cutoff edge (164 nm) and the largest birefringence (≥0.168 at 266 nm) among all the reported borate-based DUV birefringent materials.
Answer: {"reason": The text are describing a DUV birefringent crystal LiBO2 and include cutoff edge, birefringence properties","result": true}
Text: Polycrystalline samples of Pb3B6O11F2 were prepared via solid-state reaction techniques. Initially, PbO (Tianjin Baishi Chemical Reagent Co.. Ltd.. 99.0%), H3BO3 (Beijing Chemical Industry Co.. Ltd.. 99.5%), and PbF2 (Tianjin Chemical Co.. Ltd.. 99.5%) were taken in stoichiometric proportions, mixed thoroughly, and preheated in a platinum crucible at 250 °C for 3 h to decompose the water; the products were cooled to room temperature and ground again.
Answer: {"reason": "The text does not contain any property description of nlo","result": false}

Note that response format shold be json which is consistent with {"reason": ..., "result": ...}
...
# asyncio.run(extraction.extract(sample_size=30, pydantic_model=Compounds))
asyncio.run(extraction.classify(sample_size=30, pydantic_model=Compounds))
