...
'''
# TODO async version is not finished, I need to overwrite `aadd_texts` method in `Chroma`
...
from sisyphus.patch import OpenAIEmbeddingThrottle
...
async def asupervisor(file_folder, collection_name, batch_size: int = 10):
    ...
    db = chroma.Chroma(collection_name, client=client, embedding_function=embedding)
    file_paths = glob.glob(os.path.join(file_folder, '*.html'))
    for i in range(0, len(file_paths), batch_size):
        batch = file_paths[i: i+batch_size]
        coros = [aembed_doc(file_path, record_manager, db) for file_path in batch]
